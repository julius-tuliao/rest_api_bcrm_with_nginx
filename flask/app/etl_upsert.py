...
class ETL:
    def replicate(self, bank_name):
        ...
        SELECT_AMEYO_RECORDS = "SELECT ch_code,ch_name,account_number,principal,endorsement_date,\
                ...
                Where LENGTH(ch_code) > 0 AND LENGTH(ch_name) > 0 AND LENGTH(outstanding_balance) > 0 AND LENGTH(account_number) > 0 AND (updatedon::date > current_date - interval '150 HOURS' or date_added > current_date - interval '24 HOURS') "
