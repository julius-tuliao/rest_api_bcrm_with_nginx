...
class ETLLeads:
    ...
    def replicate(self):
        ...
        main_db_conn = main_db.getconn()
        main_db_cursor = main_db_conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        main_db_cursor.execute( self.SELECT_SOURCE_AND_DESTINATION, (self.bank_name,))
        ...
        destination_db = AWSConnectionPool(destination_db_name).getInstance()
        destination_db_conn = destination_db.getconn()
        ...
        for sources in destination_and_sources:
            ...
            try:
                source_db = AMEYOConnectionPool(source_db_name).getInstance()
                source_db_conn = source_db.getconn()
                ...
                source_db_cursor.execute(self.SELECT_AMEYO_RECORDS)
                ...
                for row in data:
                    ...
                    destination_db_cursor.execute( self.SELECT_USER_ID, (agent_code,))
                    ...
                    if user is not None:
                        ...
                        destination_db_cursor.execute(self.SELECT_IF_ROW_EXIST, (ch_code,))
                        ...
                        if destination_db_cursor.fetchone() is None:
                            ...
                            destination_db_cursor.execute(self.INSERT_ETL_LEADS, 
                            ...
                            for row in additional_info:
                                for key in row.keys():
                                    ...
                                    if value != None:   
                                        ...
                                        destination_db_cursor.execute(self.SELECT_DYNAMIC_ID,(client_id,key))

                                        if destination_db_cursor.rowcount > 0:
                                            dynamic_id = destination_db_cursor.fetchone()[0]

                                            destination_db_cursor.execute(self.INSERT_DYNAMIC_ROW,(leads_id,dynamic_id,value))                     
                            ...
                            destination_db_cursor.execute(self.UPDATE_ETL_LEADS, 
                                (client_id,user_id,acct_number,ch_code,ch_name,principal,interest,endo_date,lpd, \
                                lpa, cycle,credit_limit,product_type,work_phone,ob,new_ob,primary_add,address1,address2,address3,address4,\
                                phone1,email,employer,birthday,mobile_phone,new_cutoff,new_cutoff,secondary_add,tertiary_add, \
                ...
                source_db.putconn(source_db_conn)        
        ...
        destination_db.putconn(destination_db_conn)
